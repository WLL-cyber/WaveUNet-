...
from nets.EMA_attention_module import EMA
...
class UNet3Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, bilinear=True, feature_scale=4,
        ...
        self.down_wt1 = Down_wt(filters[0], filters[0])  # 使用 Down_wt 替换 MaxPool2d

        self.conv2 = unetConv2(filters[0], filters[1], self.is_batchnorm)
        self.down_wt2 = Down_wt(filters[1], filters[1])  # 使用 Down_wt 替换 MaxPool2d

        self.conv3 = unetConv2(filters[1], filters[2], self.is_batchnorm)
        self.down_wt3 = Down_wt(filters[2], filters[2])  # 使用 Down_wt 替换 MaxPool2d

        self.conv4 = unetConv2(filters[2], filters[3], self.is_batchnorm)
        self.down_wt4 = Down_wt(filters[3], filters[3])  # 使用 Down_wt 替换 MaxPool2d
        ...
        self.ema4d = EMA(self.UpChannels)  # 加入 EMA 注意力模块
        ...
        self.ema3d = EMA(self.UpChannels)  # 加入 EMA 注意力模块
        ...
        self.ema2d = EMA(self.UpChannels)  # 加入 EMA 注意力模块
        ...
        self.ema1d = EMA(self.UpChannels)  # 加入 EMA 注意力模块
    ...
    def forward(self, inputs):
        ...
        h2 = self.down_wt1(h1)  # 使用 Down_wt 进行下采样
        h2 = self.conv2(h2)  # h2->160*160*128

        h3 = self.down_wt2(h2)  # 使用 Down_wt 进行下采样
        h3 = self.conv3(h3)  # h3->80*80*256

        h4 = self.down_wt3(h3)  # 使用 Down_wt 进行下采样
        h4 = self.conv4(h4)  # h4->40*40*512

        h5 = self.down_wt4(h4)  # 使用 Down_wt 进行下采样
